...
@Component
public class MakeEmployeeOrderAction implements Action {
    ...
    @Autowired
    public MakeEmployeeOrderAction(OrderService orderService, DeliveryPointService deliveryPointService, Session session) {
    ...
    public int perform() {
        try {
            ...
            if (needSpendBonuses) {
                System.out.println("Введите логин заказчика:");
                customerLogin = in.nextLine();
            }

            MakeOrderDTO order = MakeOrderDTO.builder().employeeLogin(session.getLogin()).customerLogin(customerLogin)
